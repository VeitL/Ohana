...
struct CoconutLogView: View {
    ...
    private func filterChip(id: String?, emoji: String, name: String) -> some View {
        ...
        Button {
            ...
            .background(isSelected ? Color.goLime : Color.white.opacity(0.08), in: Capsule())
        }
        .buttonStyle(.plain)
    ...
    private func logRow(log: CoconutLogEntry) -> some View {
        ...
        HStack(spacing: 14) {
            ZStack {
                Circle()
                    .fill((isEarning ? Color.goLime : Color.goRed).opacity(0.12))
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(spacing: 6) {
                    if isSystem {
                        ...
                        Text(log.actorName ?? "")
                            .font(.system(size: 10, weight: .semibold, design: .rounded))
                            .foregroundStyle(Color.goLime.opacity(0.9))
                            .padding(.horizontal, 6).padding(.vertical, 2)
            ...
            HStack(spacing: 3) {
                Text(isEarning ? "+\(log.amount)" : "\(log.amount)")
                    .font(.system(size: 18, weight: .black, design: .rounded))
                    .foregroundStyle(isEarning ? Color.goLime : Color.goRed)
