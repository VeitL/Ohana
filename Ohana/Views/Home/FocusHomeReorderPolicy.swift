...
enum FocusHomeReorderPolicy {
    ...
    static func update(
        ...
        while targetIndex > 0 {
            let previousTopY = slotTopY(targetIndex - 1, workingCards.count)
            guard draggedTopY < previousTopY else { break }
            targetIndex -= 1
        }

        while targetIndex < workingCards.count - 1 {
            let nextTopY = slotTopY(targetIndex + 1, workingCards.count)
            guard draggedTopY > nextTopY else { break }
            targetIndex += 1
        }
        ...
        let currentSlotTopY = slotTopY(targetIndex, workingCards.count)
        return FocusHomeReorderUpdate(
            cards: workingCards,
            dragOffset: draggedTopY - currentSlotTopY,
            didMove: didMove
