//
//  HomeV2SnapshotBuilder.swift
//  Ohana
//
//  Read-model aggregation for Home V2. Views should render the snapshot, not
//  derive dashboard state from SwiftData collections inside body.
//

import Foundation

struct HomeV2SourceState {
    let pets: [Pet]
    let humans: [Human]
    let plants: [Plant]
...
enum HomeV2SnapshotBuilder {
    static func build(from source: HomeV2SourceState, now: Date = Date()) -> HomeV2Snapshot {
    ...
    static func signature(for source: HomeV2SourceState) -> String {
        [
...
enum HomeV2PreloadPlanner {
    static func avatarSignature(for source: HomeV2SourceState) -> String {
        (
