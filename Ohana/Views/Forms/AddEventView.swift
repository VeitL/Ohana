    ...
    var body: some View {
        OhanaSheetWrapper(title: "添加事件", onDismiss: { dismiss() }) {
            VStack(spacing: 20) {
                ...
                // 时间（非全天：参考 Google 日历 — 开始/结束各一行，左日期、右时间）
                VStack(spacing: 14) {
                    Toggle("全天事件", isOn: $isAllDay)
                        ...
                        .onChange(of: isAllDay) { _, allDay in
                            ...
                            if allDay {
                                ...
                                if hasEndDate { endDate = cal.startOfDay(for: endDate) }
                            }
                        }
                ...
                .onChange(of: startDate) { _, _ in
                    guard !isAllDay, hasEndDate, endDate < startDate else { return }
                    endDate = Calendar.current.date(byAdding: .hour, value: 1, to: startDate) ?? startDate
                }
                        ...
                        HStack(spacing: 8) {
                            ...
                            ForEach(plants) { plant in
                                Button {
                                    ...
                                    .foregroundStyle(relatedEntityId == plant.id.uuidString ? .white : .primary)
                                    .padding(.horizontal, 14)
                                    .padding(.vertical, 8)
                                    .background(
                                        relatedEntityId == plant.id.uuidString ? Color.arkMint : Color.gray.opacity(0.15),
                                        in: Capsule()
                                    )
                                }
                            }
