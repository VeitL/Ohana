    ...
    var body: some View {
        Button(action: onTap) {
            VStack(spacing: 8) {
                ZStack {
                    Circle()
                        .fill(plant.needsWatering ? Color.goLime.opacity(0.2) : .white.opacity(0.15))
...
struct HomeSectionManageSheet: View {
    ...
    @AppStorage("home_section_order") private var sectionOrderRaw: String = "petCards,islandStats,todayTasks,quickAccess"
    ...
    private func loadState() {
        let order = sectionOrderRaw.split(separator: ",").map(String.init)
