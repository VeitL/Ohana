...
struct SmartTaskEngine {
    static func topTask(pets: [Pet], reminders: [Reminder]) -> SmartTask {
        ...
        // 5. 默认：今日一切安好
        let petName = pets.first?.name ?? "家人们"
        return SmartTask(
            emoji: "✨",
            title: "今日一切安好",
...
struct SmartTodayCard: View {
    ...
    private var textColor: Color {
        if task.accentColor == .goLime || task.accentColor == .goYellow {
            return Color.arkInk
        }
        return .white
    }
            ...
            HStack(alignment: .top, spacing: 14) {
                ...
                Button {
                    ...
                    Text(didFeed ? "✓ 已投喂" : "已投喂 🍖")
                        ...
                        .background(didFeed ? Color.goLime.opacity(0.5) : Color.goLime, in: Capsule())
                }
                .disabled(didFeed)
            }
