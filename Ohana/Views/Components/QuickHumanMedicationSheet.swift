...
private enum HumanMedicationQuickCatalog {
    static func names(for countryCode: String) -> [String] {
        ...
        case "US", "GB":
            return ["Ibuprofen", "Acetaminophen", "Vitamin D", "Cetirizine", "Melatonin", "Omeprazole"]
        case "DE":
            return ["Ibuprofen", "Paracetamol", "Vitamin D", "Cetirizin", "Magnesium", "Omeprazol"]
        case "JP":
            return ["イブプロフェン", "アセトアミノフェン", "ビタミンD", "ロキソニン", "葛根湯", "整腸剤"]
        case "HK", "TW":
            return ["布洛芬", "乙酰氨基酚", "維生素D", "益生菌", "褪黑素", "抗敏感藥"]
        default:
            return ["布洛芬", "对乙酰氨基酚", "维生素D", "益生菌", "褪黑素", "抗过敏药"]
        }
    }
}
...
struct QuickHumanMedicationSheet: View {
    ...
    private var nameBlock: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(l.tr(zh: "药品名称", en: "Medication name", de: "Medikamentenname"))
                ...
                .foregroundStyle(Color.ohanaSecondaryText)
            TextField(l.tr(zh: "例如：维生素 D", en: "e.g. Vitamin D", de: "z. B. Vitamin D"), text: $medicationName)
            ...
            HStack(spacing: 8) {
                ForEach(QuickHumanMedicationForm.allCases) { item in
                    Button {
                        ...
                        .frame(height: 56)
                        .background(form == item ? Color.goPrimary : Color.ohanaCardSurface, in: RoundedRectangle(cornerRadius: 18, style: .continuous))
                    }
                    .buttonStyle(ScaleButtonStyle())
                }
            }
    ...
    private var noteBlock: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(l.tr(zh: "备注（可选）", en: "Note (optional)", de: "Notiz (optional)"))
                .font(OhanaFont.caption(.black))
                .foregroundStyle(Color.ohanaSecondaryText)
            TextField(l.tr(zh: "例如：饭后、睡前", en: "After meal, before bed", de: "Nach dem Essen, vor dem Schlafen"), text: $notes)
    ...
    private func doseMinutes(for frequency: MedicationFrequency, firstMinute: Int) -> [Int] {
        ...
        case .daily, .weekly:
            return [firstMinute]
        case .twiceDaily:
            return HumanMedicationScheduleMetadata.normalizedDoseMinutes([firstMinute, firstMinute + 12 * 60])
        case .threeTimesDaily:
            return HumanMedicationScheduleMetadata.normalizedDoseMinutes([firstMinute, firstMinute + 6 * 60, firstMinute + 12 * 60])
