...
struct GlobalWalkBanner: View {
    ...
    private func draggableBubble(pet: Pet, geo: GeometryProxy) -> some View {
        ...
        return ZStack {
            Circle()
                ...
                .overlay(Circle().strokeBorder(Color.goLime.opacity(0.5), lineWidth: 2))
                .shadow(color: Color.goLime.opacity(0.35), radius: 12)
            VStack(spacing: 1) {
    ...
    private func expandedCard(pet: Pet) -> some View {
        VStack(spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        HStack(spacing: 5) {
                            if case .paused = mgr.phase {
                                ...
                                Circle().fill(Color.goLime).frame(width: 7, height: 7)
                                    .shadow(color: Color.goLime.opacity(0.8), radius: 4)
                                Text("巡岛中")
                                    .font(.system(size: 11, weight: .bold, design: .rounded))
                                    .foregroundStyle(Color.goLime)
                            }
            ...
            // 数据行
            HStack(spacing: 0) {
                walkStatCell(label: "时长", accent: .goLime) {
                    TimelineView(.periodic(from: .now, by: 1)) { _ in
                        Text(formatElapsed(mgr.elapsedTime))
