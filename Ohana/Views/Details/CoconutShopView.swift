...
struct CoconutShopView: View {
            ...
            ForEach(ShopItem.ShopCategory.allCases, id: \.self) { cat in
                Button {
                    ...
                    Text(cat.rawValue)
                        ...
                        .background(
                            selectedCategory == cat ? Color.goLime : Color.white.opacity(0.08),
                            in: Capsule()
                        )
                }
                .buttonStyle(.plain)
    ...
    private func successToast(_ item: ShopItem) -> some View {
        VStack {
            ...
            .background(Color.goLime, in: RoundedRectangle(cornerRadius: 14, style: .continuous))
            .shadow(color: Color.goLime.opacity(0.5), radius: 16, x: 0, y: 4)
            .padding(.horizontal, 24)
            .padding(.top, 60)
            Spacer()
        }
