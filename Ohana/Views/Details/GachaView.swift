        ...
        NavigationStack {
            ZStack {
                ...
                ForEach(particles, id: \.id) { p in
                    Circle()
                        .fill(Color.goLime.opacity(p.opacity))
                        .frame(width: 6, height: 6)
                        .position(x: p.x, y: UIScreen.main.bounds.height * 0.42)
                        .blur(radius: 2)
                }
    ...
    private var gachaMachine: some View {
        VStack(spacing: 24) {
            ...
            Button {
                ...
                .padding(.vertical, 16)
                .background(
                    canRoll ? Color.goLime : Color.white.opacity(0.08),
                    in: RoundedRectangle(cornerRadius: 18, style: .continuous)
                )
                .shadow(color: canRoll ? Color.goLime.opacity(0.5) : .clear, radius: 20, x: 0, y: 6)
            }
            .buttonStyle(.plain)
