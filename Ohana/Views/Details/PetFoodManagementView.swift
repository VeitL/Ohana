...
struct PetFoodManagementView: View {
    ...
    @State private var calcLifeStage: Int = 2   // 0=幼年, 1=青年, 2=成年, 3=绝育成年, 4=老年
            ...
            if let runOut = pet.casualEstimatedRunOutDate {
                ...
                HStack(spacing: 8) {
                    Text("🍖")
                        .font(.system(size: 20))
                    VStack(alignment: .leading, spacing: 2) {
                        let accent: Color = remaining <= 7 ? .goRed : remaining <= 14 ? .goYellow : .goPrimary
                ...
                .padding(12)
                .background(
                    (remaining <= 7 ? Color.goRed : remaining <= 14 ? Color.goYellow : Color.goPrimary).opacity(0.08),
                ...
                VStack(spacing: 10) {
                    ...
                    Button { saveStock() } label: {
                        Text("保存库存信息")
                            ...
                            .background(Color.goPrimary, in: RoundedRectangle(cornerRadius: 12))
                    }
                    .buttonStyle(.plain)
                }
            } else {
                    ...
                    Button { setAsDefault.toggle() } label: {
                        HStack(spacing: 6) {
                            Image(systemName: setAsDefault ? "checkmark.circle.fill" : "circle")
                                .font(.system(size: 14)).foregroundStyle(setAsDefault ? Color.goPrimary : .primary.opacity(0.3))
                            Text("设为默认单次喂食量")
                                .font(.system(size: 12, weight: .medium, design: .rounded))
                                .foregroundStyle(.primary.opacity(0.5))
                        }
                    ...
                    Button { setWaterAsDefault.toggle() } label: {
                        HStack(spacing: 6) {
                            Image(systemName: setWaterAsDefault ? "checkmark.circle.fill" : "circle")
                                .font(.system(size: 14)).foregroundStyle(setWaterAsDefault ? Color.goPrimary : .primary.opacity(0.3))
                            Text("设为默认加水量")
                                .font(.system(size: 12, weight: .medium, design: .rounded))
                                .foregroundStyle(.primary.opacity(0.5))
                        }
    ...
    private var calculatorSection: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Button {
                ...
                HStack(spacing: 10) {
                    ZStack {
                        Circle().fill(Color.goPrimary.opacity(0.2)).frame(width: 32, height: 32)
                ...
                VStack(spacing: 16) {
                                ...
                                ForEach(lifeStageLabels.indices, id: \.self) { i in
                                    Button { calcLifeStage = i } label: {
                                        Text(lifeStageLabels[i])
                                            ...
                                            .foregroundStyle(calcLifeStage == i ? .black : .primary)
                                            .padding(.horizontal, 10).padding(.vertical, 6)
                                            .background(calcLifeStage == i ? Color.goPrimary : Color.primary.opacity(0.08), in: Capsule())
                                    }
                                    .buttonStyle(.plain)
                                }
                            }
                    ...
                    if let result = rerResult {
                        HStack(spacing: 0) {
                            ...
                            // 一键填入
                            Button {
                                let mid = (result.low + result.high) / 2
                                dailyGramsInput = String(format: "%.0f", mid)
                                withAnimation { showingCalculator = false }
