...
nonisolated enum PlantCatalog {
    ...
    static func searchResults(_ query: String, limit: Int? = nil) -> [PlantCatalogSearchResult] {
        ...
        guard !normalized.isEmpty else {
            let results = entries.map {
                PlantCatalogSearchResult(entry: $0, score: 0, matchSummary: "常见室内植物")
            }
    ...
    private static func searchScore(
        ...
        add(entry.commonName, weight: 24, label: "俗名")
        add(entry.latinName, weight: 22, label: "拉丁名")
        addBest(entry.aliases, weight: 20, label: "别名")
        add(entry.careDifficulty, weight: 10, label: "难度")
        add(entry.lightRequirement.displayName, weight: 9, label: "光照")
        add(entry.lightRequirement.rawValue, weight: 9, label: "光照")
        add(lightSearchText(for: entry), weight: 10, label: "光照")
        add(entry.wateringPreference, weight: 7, label: "浇水")
        add(entry.humidity, weight: 7, label: "湿度")
        add(entry.soil, weight: 6, label: "土壤")
        add(entry.toxicity, weight: 6, label: "安全")
        add(safetySearchText(for: entry), weight: 8, label: "安全")
        return (score, labels.isEmpty ? ["匹配"] : labels)
    }
