...
nonisolated enum FocusHomeFabShortcutPolicy {
    ...
    static func petShortcut(from option: QuickActionPickerCatalog.Option) -> ExpandedCardFabShortcut {
        switch option.id {
        case "health":
            ExpandedCardFabShortcut(label: option.label, icon: option.icon, action: .detail(.health))
        case "expense":
            ExpandedCardFabShortcut(label: option.label, icon: option.icon, action: .detail(.expense))
        case "weight":
            ExpandedCardFabShortcut(label: option.label, icon: option.icon, action: .detail(.weight))
        default:
            ExpandedCardFabShortcut(label: option.label, icon: option.icon, action: .quick(option.id))
        }
    }
