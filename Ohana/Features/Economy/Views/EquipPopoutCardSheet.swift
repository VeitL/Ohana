...
struct EquipPopoutCardSheet: View {
            ...
            if isPopoutActive {
                Button {
                    ...
                    commandQueue.enqueue(.petCardAppearance(petID: pet.id, action: "restoreClassic")) {
                        _ = RewardEconomyCommandExecutor(context: modelContext, services: appServices).restoreClassicPetCard(
                            pet: pet,
    ...
    private func savePopout() {
        ...
        commandQueue.enqueue(.petCardAppearance(petID: pet.id, action: "enablePopout")) {
            _ = RewardEconomyCommandExecutor(context: modelContext, services: appServices).enablePetPopoutCard(
                pet: pet,
                imageData: data,
                sourceRaw: sourceRaw,
