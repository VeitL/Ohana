...
struct ReminderObservabilityContentView: View {
    let snapshot: ReminderObservabilitySnapshot
    ...
    @State private var preparedDiagnosticCSV = "metric,value"
    ...
    var body: some View {
        ...
        .onAppear { prepareDiagnosticExport() }
        .onChange(of: snapshot) { prepareDiagnosticExport() }
        .onChange(of: appLanguage) { prepareDiagnosticExport() }
    }
    ...
    private var healthHeader: some View {
        ...
        return VStack(alignment: .leading, spacing: 14) {
            ...
            if appServices.commerce.allows(.extendedTrends) {
                ShareLink(item: preparedDiagnosticCSV) {
                    Label(
                ...
                .accessibilityIdentifier("reminder-diagnostics-export")
            }
        }
        .padding(16)
    ...
    private var notificationPermissionCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 10) {
                ...
                metric(l.tr(zh: "App 待办", en: "App pending", de: "App offen"), "\(snapshot.pendingCount)", .goTeal)
            }
    ...
    private var statusBreakdownCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 10) {
                metric(l.tr(zh: "未来待办", en: "Upcoming", de: "Anstehend"), "\(snapshot.upcomingCount)", .goPrimary)
                metric(l.tr(zh: "已过期", en: "Overdue", de: "Ueberfaellig"), "\(snapshot.overdueCount)", snapshot.overdueCount == 0 ? .goTeal : .goOrange)
                metric(l.tr(zh: "失败", en: "Failed", de: "Fehlgeschlagen"), "\(snapshot.failedCount)", snapshot.failedCount == 0 ? .goTeal : .goRed)
            }
            HStack(spacing: 10) {
                metric(l.tr(zh: "本周完成", en: "Done this week", de: "Diese Woche erledigt"), "\(snapshot.completedThisWeekCount)", .goPrimary)
                metric(l.tr(zh: "总提醒", en: "Total reminders", de: "Reminder gesamt"), "\(snapshot.totalCount)", .secondary)
            }
        }
        .padding(16)
        .goTranslucentCard(cornerRadius: OhanaRadius.cardSoft)
    ...
    private var schedulingLedgerCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if snapshot.actionCounts.isEmpty {
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if snapshot.riskItems.isEmpty {
                ...
                ForEach(snapshot.riskItems) { reminder in
                    HStack(spacing: 10) {
                        Image(systemName: reminder.status == .failed ? "xmark.octagon.fill" : "clock.badge.exclamationmark.fill")
                            .foregroundStyle(reminder.status == .failed ? Color.goRed : Color.goOrange)
    ...
    private var reminderHealthScore: (value: Int, color: Color, message: String) {
        ...
        score -= min(snapshot.overdueCount * 8, 32)
        score -= min(snapshot.failedCount * 10, 30)
        if pendingNotificationCount == 0, snapshot.upcomingCount > 0 { score -= 12 }
    ...
    private func prepareDiagnosticExport() {
        ...
        let values: [(String, String)] = [
            ...
            ("system_queued", String(pendingNotificationCount)),
            ("app_pending", String(snapshot.pendingCount)),
            ("upcoming", String(snapshot.upcomingCount)),
            ("overdue", String(snapshot.overdueCount)),
            ("failed", String(snapshot.failedCount)),
            ("completed_last_7_days", String(snapshot.completedThisWeekCount)),
            ("total", String(snapshot.totalCount))
        ]
        lines.append(contentsOf: values.map {
            "\(HouseholdInsightExport.csvCell($0.0)),\(HouseholdInsightExport.csvCell($0.1))"
        })
        lines.append(contentsOf: snapshot.actionCounts.map { stat in
            let metricName = "action_\(stat.action)"
            return "\(HouseholdInsightExport.csvCell(metricName)),\(stat.count)"
        })
        preparedDiagnosticCSV = lines.joined(separator: "\n")
    }
