...
nonisolated extension DataBackupManager {
    ...
    func decodeRecycleBinBatch(_ dto: RecycleBinBatchBackup) -> RecycleBinBatch {
        RecycleBinBatch(
            id: UUID(uuidString: dto.id) ?? UUID(),
            kindRaw: dto.kindRaw,
            title: dto.title,
            subtitle: dto.subtitle,
            sourceEntityName: dto.sourceEntityName,
            sourceEntityId: dto.sourceEntityId,
            trashedAt: parseDate(dto.trashedAt) ?? Date(),
            trashExpiresAt: parseDate(dto.trashExpiresAt) ?? Date(),
            trashedByHumanId: dto.trashedByHumanId,
            metadataJSON: dto.metadataJSON
        )
    }

    @MainActor
    func applyTrashStates(_ states: [TrashStateBackup], context: ModelContext) throws {
        for state in states {
            switch state.entityName {
            case String(describing: Pet.self):
                try applyTrashState(state, to: Pet.self, context: context)
            case String(describing: Human.self):
                try applyTrashState(state, to: Human.self, context: context)
            case String(describing: Event.self):
                try applyTrashState(state, to: Event.self, context: context)
            case String(describing: Plant.self):
                try applyTrashState(state, to: Plant.self, context: context)
            case String(describing: PetCareLog.self):
                try applyTrashState(state, to: PetCareLog.self, context: context)
            case String(describing: PetPottyLog.self):
                try applyTrashState(state, to: PetPottyLog.self, context: context)
            case String(describing: PetWalkLog.self):
                try applyTrashState(state, to: PetWalkLog.self, context: context)
            case String(describing: PetWeightLog.self):
                try applyTrashState(state, to: PetWeightLog.self, context: context)
            ...
            case String(describing: PetFoodRecord.self):
                try applyTrashState(state, to: PetFoodRecord.self, context: context)
            case String(describing: PetDocument.self):
                try applyTrashState(state, to: PetDocument.self, context: context)
            case String(describing: PetMilestone.self):
                try applyTrashState(state, to: PetMilestone.self, context: context)
            case String(describing: PetPhotoLog.self):
                try applyTrashState(state, to: PetPhotoLog.self, context: context)
            case String(describing: PetInsurance.self):
                try applyTrashState(state, to: PetInsurance.self, context: context)
            case String(describing: PetMedication.self):
                try applyTrashState(state, to: PetMedication.self, context: context)
            default:
                continue
            }
        }
    ...
    @MainActor
    private func applyTrashState<T: PersistentModel & RecycleBinSoftDeletable>(
        _ state: TrashStateBackup,
        to _: T.Type,
        context: ModelContext
    ) throws {
        guard let id = UUID(uuidString: state.id) else { return }
        guard let model = try context.fetch(FetchDescriptor<T>()).first(where: { $0.id == id }) else { return }
        model.trashedAt = parseDate(state.trashedAt)
        model.trashExpiresAt = parseDate(state.trashExpiresAt)
        model.trashBatchId = state.trashBatchId
        model.trashedByHumanId = state.trashedByHumanId
    }
