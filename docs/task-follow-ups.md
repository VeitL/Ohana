...
- Status: Open.
- Why still open: `CareLedgerAnalysisView` now includes event-type distribution, actor/family-member breakdown, and a daily trend card backed by `CareLedgerAnalysisScreenModel`, but Hygiene and QuickCare status feedback are still not fully unified on one overdue/status source.
- Next action: share the same overdue/status feedback source between Hygiene and QuickCare, then close the remaining display-source mismatch.
