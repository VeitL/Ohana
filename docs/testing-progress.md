...
| Economy | 6 | 🔵 | 2026-06-12 | 重新对抗复审发现 P1：奖励归属仍有 5 个 executor 缺口，见 TFU-20260613-009；真实 UI 抽查见统一 track list | `1679ddd66` | 规则书见 `docs/specs/Economy-logic.md`；线下兑现入口首发门关闭，冻结钱包拒绝成就 / 商店 / 金库 / 奖励写入，特殊奖励与 legacy 兼容奖励无 actor 时归属 active human 且不写 system，隐私钱包计入总额但隐藏明细，离世 / 回收钱包退出活跃财富且历史保留；2026-06-13 复审修复轮已完成：重复喂药 / 时刻奖励进入预算冷却管线、首宠欢迎奖励不写 system、商店支持岛屿合资、兑换服务层硬门、财富趋势排除 system、遛狗先存事实再发奖；2026-06-13 重新对抗复审未清零：`DashboardRecordCommands` 体重、`RewardEconomyCommands` 补签、`ExpenseCommands` 宠物花费、`PetHealthCommands` 健康、`PetMilestoneCommands` 里程碑仍未把业务 executor 显式传入奖励管线，会回落 active human，撞 D3/ECO-018；`scripts/audit-economy-boundaries.sh --all` 复现 5 条 baseline warning；需修复后重新复审才可标 🏁 |
| Medication | 7 | ⬜ | | | | |
...
| GAP-2 回收站 | P0 上架前 | 🔵 | 成员+珍贵档案软删 30 天可恢复；流水直删但写 tombstone；涉及 schema 升版 | `8bddfe1a6` | 开工：2026-06-12；门禁：`scripts/module-exit-gate.sh` PASS；自动验收通过，真实 UI 追踪项见 `docs/planning/gap-acceptance-track-list.md#gap-2-回收站`；schema 升至 `ArkSchemaV69`；2026-06-13 对抗复审 P1 已修：30 天到期改为服务层硬边界，恢复 Event/Reminder 后重新排未来本地通知，Pet 期满 purge 为级联子记录写 tombstone，单条流水直删路径写 CloudSync tombstone；2026-06-13 重新对抗复审确认 RecycleBinService 本体四条 P1 已有测试覆盖，但全仓删除边界仍有 P1：Calendar whole-event / recurrence 删除物理删 `Event` 无 tombstone，PetCare/Hygiene/DashboardRecords 删除业务 fact 时物理删 `CareLedgerEvent` 无 tombstone，CatCare undo 物理删 `Event` / `PetHygieneLog` 无 tombstone；这些命中 D16/G5/G6 与 TFU-20260613-009，需修复后重新复审才可标 🏁；P2 余留仍见 TFU-20260613-003~005；未改路由 / 启动路径 / CloudKit |
