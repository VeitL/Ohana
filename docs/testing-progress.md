...
| 6 | 大模块（Feeding/Members/Oasis/Settings/Health/Economy） | 🟡 | Feeding 门禁通过并提交：`b49134977`；Members 门禁通过并提交：`ead1e5fe4`；Oasis 门禁通过，提交待回填；Settings/Health/Economy 待启动 |
...
| Oasis | 6 | 🟢 | 2026-06-12 | 无 | 待提交 | 开工：2026-06-12；规则书见 `docs/specs/Oasis-logic.md`；当前主人钱包门、预算/冷却产出、一次性幂等奖励、休眠可唤回语义已落地；`scripts/module-exit-gate.sh` PASS；真实 UI 抽查见 `docs/planning/gap-acceptance-track-list.md#phase-6-oasis`；未改 schema / 路由 / 启动路径 / CloudKit |
| Settings | 6 | ⬜ | | | | |
| Health | 6 | ⬜ | | | | |
| Economy | 6 | ⬜ | | | | |
| Medication | 7 | ⬜ | | | | |
