...
| --- | --- | --- | --- |
| Review-gate / likely implemented locally | TFU-20260612-014 | This is still counted as Open P1 because its close condition requires broader cleanup/review evidence. Its entry already records local repair evidence. | Do a dedicated closure pass: rerun the relevant current-code guards and close or split only the findings that are still real. Do not treat it as fresh implementation work until review disproves the local evidence. |
| Actual code / product-policy work still likely needed | None identified in this pass | TFU-20260612-019, TFU-20260614-017, TFU-20260614-016, TFU-20260614-015, TFU-20260614-018, TFU-20260614-019, TFU-20260615-001, and TFU-20260614-013 were verified as covered by the current route/command/test matrix and closed below. | Next fresh review should look for new current-code findings rather than assuming another known code P1 exists. |
...
Operational read after closing TFU-20260612-019, TFU-20260614-017, TFU-20260614-016, TFU-20260614-015, TFU-20260614-018, TFU-20260614-019, TFU-20260615-001, and TFU-20260614-013: the raw Open P1 count is 4,
...
- Status: Done - current-head closure review verified Feeding plan/stock
  lifecycle gates, effective actor policy, and module-gate follow-up coverage.
- Priority: P1
...
- Closed: 2026-06-25 current-head closure review found no remaining Feeding
  plan/stock lifecycle or actor-policy P1 in the covered first-release surfaces.
  Validation:
  `scripts/test-simulator.sh -only-testing:OhanaTests/ManualFeedCommandTests`
  PASS (21 Swift Testing tests, iPhone 17 simulator, xcresult
  `/var/folders/9j/7ldcxzn91d947mg4p_7wxmz40000gn/T/OhanaDerivedData/main-b6cf423d5931-tests/Logs/Test/Test-Ohana-2026.06.25_06-34-04-+0200.xcresult`);
  the current-head Home and representative domain suites also passed during this
  closure pass:
  `HomeCommandExecutorTests` + `RecurringFindingsRepairTests` PASS (201 tests,
  xcresult `Test-Ohana-2026.06.25_06-26-18-+0200.xcresult`) and
  `MemberLifecycleGateTests` + care/economy representative suites PASS (154
  tests, xcresult `Test-Ohana-2026.06.25_06-32-28-+0200.xcresult`). Covered
  examples include deceased pet feed/stock no-op, stock expense effective actor
  fallback for deceased payer, stock reminder rebuild/delete behavior, and
  feed-plan command no-op paths.
...
- Status: Done - current-head closure review verified memorial-safe versus
  active-only command/route membrane and representative no-derived-effects
  behavior.
- Priority: P1
...
- Closed: 2026-06-25 current-head closure review found no remaining memorial
  active-only / memorial-safe P1 in the covered first-release surfaces.
  Validation:
  `scripts/test-simulator.sh -only-testing:OhanaTests/MemberLifecycleGateTests
  -only-testing:OhanaTests/CareCompletionChokepointCharacterizationTests
  -only-testing:OhanaTests/CareDerivationExecutorSuccessCharacterizationTests
  -only-testing:OhanaTests/RecurringFindingsRepairTests
  -only-testing:OhanaTests/ReminderActionCoordinatorTests
  -only-testing:OhanaTests/CatCareCommandTests` PASS (154 Swift Testing tests,
  iPhone 17 simulator, xcresult
  `/var/folders/9j/7ldcxzn91d947mg4p_7wxmz40000gn/T/OhanaDerivedData/main-b6cf423d5931-tests/Logs/Test/Test-Ohana-2026.06.25_06-32-28-+0200.xcresult`).
  The run includes `deceasedFeatureHubsExposeOnlyMemorialSafeDestinations`,
  deceased pet feed/stock/water/care-plan/walk/derived wrapper no-op,
  deceased pet memorial calendar/photo/moment allowed without care/economy
  derivation, deceased human note with reminder as memorial-content-only, and
  active-only pet/human command no-op coverage.

### TFU-20260614-018 - Resolve unwritable executors consistently across fact, ledger, and reward

- Status: Done - current-head closure review verified Home and non-Home actor
  consistency guards across fact, ledger, reward, budget, and target no-op paths.
- Priority: P1
...
- Closed: 2026-06-25 current-head closure review found no remaining executor
  actor-consistency P1 in the covered first-release care-family surfaces. Home
  validation reused the current-head
  `scripts/test-simulator.sh -only-testing:OhanaTests/HomeCommandExecutorTests
  -only-testing:OhanaTests/RecurringFindingsRepairTests` PASS (201 Swift Testing
  tests, iPhone 17 simulator, xcresult
  `/var/folders/9j/7ldcxzn91d947mg4p_7wxmz40000gn/T/OhanaDerivedData/main-b6cf423d5931-tests/Logs/Test/Test-Ohana-2026.06.25_06-26-18-+0200.xcresult`),
  which includes the four Home quick-action fallback guards. Non-Home validation:
  `scripts/test-simulator.sh -only-testing:OhanaTests/CareCompletionChokepointCharacterizationTests`
  PASS (24 Swift Testing tests, iPhone 17 simulator, xcresult
  `/var/folders/9j/7ldcxzn91d947mg4p_7wxmz40000gn/T/OhanaDerivedData/main-b6cf423d5931-tests/Logs/Test/Test-Ohana-2026.06.25_06-31-10-+0200.xcresult`),
  covering direct manual feed, shared care, medication dose, calendar completion,
  walk, pet weight, pet health, missing/deceased explicit executor fallback,
  and deceased-target no-op paths.

### TFU-20260614-017 - Delete PetMilestone ledger with the milestone fact

- Status: Done - current-head closure review verified creation/write cleanup,
  historical legacy-ledger delete cleanup, and regression guards.
- Priority: P1
...
- Closed: 2026-06-25 current-head closure review found no remaining
  PetMilestone ledger P1. Static guard evidence: `PetMilestoneCommands.swift`
  contains no `recordLedger(` / `careLedger.record(` write path; creation and
  seed paths create `PetMilestone` only; deletion keeps only the narrow
  `ledgerEvents(for:)` lookup for historical `legacyModelName == "PetMilestone"`
  cleanup, and `PhysicalDeletionService.petScopedLegacyReference` includes
  `PetMilestone`. Validation:
  `scripts/test-simulator.sh -only-testing:OhanaTests/HomeCommandExecutorTests
  -only-testing:OhanaTests/RecurringFindingsRepairTests` PASS (201 Swift Testing
  tests, iPhone 17 simulator, xcresult
  `/var/folders/9j/7ldcxzn91d947mg4p_7wxmz40000gn/T/OhanaDerivedData/main-b6cf423d5931-tests/Logs/Test/Test-Ohana-2026.06.25_06-26-18-+0200.xcresult`).
  A preceding exact single-test filter run executed 0 tests and is explicitly
  not counted as closure evidence. CI/GitHub inspection remains unavailable in
  this Codex session; per current user instruction, this close is based on local
  current-head evidence and durable repo docs.

### TFU-20260614-016 - Scrub deleted human executors from retained active pet facts

- Status: Done - current-head closure review verified retained active pet fact,
  ledger, backfill, and backup deleted-executor scrub.
- Priority: P1
...
- Progress: 2026-06-24 refreshed local validation:
  ...
  review，所以保持 Open。
- Closed: 2026-06-25 current-head closure review found no remaining retained
  active pet executor scrub P1. Static evidence: `deleteHumanScopedRows` calls
  `scrubSharedCareSessionsReferencingHuman` and
  `scrubRetainedPetFactsReferencingHuman` before deleting human-scoped care /
  coconut ledger rows; the retained-fact scrubber covers care, potty, hygiene,
  health, walk, weight, food, and pet expense facts plus matching care/coconut
  ledger actors. Validation:
  `scripts/test-simulator.sh -only-testing:OhanaTests/PhysicalDeletionServiceTests`
  PASS (12 Swift Testing tests, iPhone 17 simulator, xcresult
  `/var/folders/9j/7ldcxzn91d947mg4p_7wxmz40000gn/T/OhanaDerivedData/main-b6cf423d5931-tests/Logs/Test/Test-Ohana-2026.06.25_06-28-24-+0200.xcresult`);
  the executed suite includes
  `deleteHumanScrubsExecutorFromRetainedPetFactsAndLedgers`, which asserts
  retained facts keep their ids, deleted executor ids are cleared, matching
  care/coconut ledgers no longer expose the deleted actor, backfill does not
  reintroduce it, and backup projection omits it.

### TFU-20260614-015 - Clear shared-care child references when deleting a human executor

- Status: Done - current-head closure review verified shared-care child session
  detachment/survivor scrub and ledger actor consistency.
- Priority: P1
...
- Closed: 2026-06-25 current-head closure review found no remaining shared-care
  child orphan P1. Static evidence:
  `scrubSharedCareChildrenReferencingSession` covers `PetCareLog`,
  `PetPottyLog`, `PetHygieneLog`, `PetExpenseLog`, and `PetWalkLog`; it clears
  the session link when the only executor session is removed, or rewrites child
  executor metadata to the survivor when the session remains. Validation:
  `scripts/test-simulator.sh -only-testing:OhanaTests/PhysicalDeletionServiceTests`
  PASS (12 Swift Testing tests, iPhone 17 simulator, xcresult
  `/var/folders/9j/7ldcxzn91d947mg4p_7wxmz40000gn/T/OhanaDerivedData/main-b6cf423d5931-tests/Logs/Test/Test-Ohana-2026.06.25_06-28-24-+0200.xcresult`);
  the executed suite includes
  `deleteHumanDetachesSharedCareChildrenWhenOnlyExecutorSessionIsRemoved` and
  `deleteHumanScrubsSharedCareChildrenWhenSessionKeepsOtherExecutors`, covering
  both delete-session and keep-session paths plus matching care ledger actor
  consistency.
...
- Status: Done - current-head closure review verified Gacha/Shop upload builder
  coverage and human-delete lifecycle cascade; live-apply policy successor
  remains separately tracked by TFU-20260614-014.
- Priority: P1
...
- Closed: 2026-06-25 current-head closure review found no remaining local
  Gacha/Shop upload-builder or human-delete lifecycle P1. Validation:
  `scripts/test-simulator.sh -only-testing:OhanaTests/CloudSyncMetadataServiceTests`
  PASS (97 Swift Testing tests, iPhone 17 simulator, xcresult
  `/var/folders/9j/7ldcxzn91d947mg4p_7wxmz40000gn/T/OhanaDerivedData/main-b6cf423d5931-tests/Logs/Test/Test-Ohana-2026.06.25_06-35-33-+0200.xcresult`);
  current-head `PhysicalDeletionServiceTests` also PASS (12 tests, xcresult
  `Test-Ohana-2026.06.25_06-28-24-+0200.xcresult`) and covers first-release
  human cascade for `GachaOwnedItem`, `GachaDrawLog`, and `ShopPurchaseRecord`.
  Static evidence: `CloudSyncUploadBatchBuilder.localModel` has fetch cases for
  the three registered Gacha/Shop entities, and `PhysicalDeletionService` lists
  them in local cascade coverage. TFU-20260614-014 remains Open for the separate
  CloudKit 1.x live remote apply deletion-wins / parent lifecycle / natural-key
  policy.
