...
- **颜色与背景**：深色预览使用 `deep` 背景；全 app 主视觉以黑、白、灰为主。`goPrimary` 作为大面积 primary fill 在浅色解析为黑、深色解析为白；`goLime` 只用于小面积功能 icon、确认细节、焦点和选中态。全局背景必须以“浅色/深色成对”的官方黑白灰背景包选择，一次选择同时决定两种模式；浅色背景必须比浅色卡片更深一档，让白色/浅色卡片保持层次。当前官方背景对为 `goIsland`、`cleanBlueGray`、`paperCream`、`forestGlade`、`deepAmbient`；自定义照片使用同一张图片叠加深/浅色单色可读性遮罩。
- **主操作颜色纪律**：`goPrimary` 是黑/白 primary fill，`goLime` 是唯一全局小面积强调色。主 CTA 可以使用黑/白 fill；确认、关键入口、功能 icon、焦点和系统级选中态用少量 `goLime` 表达；页面背景和业务表面保持黑白灰；喂食、提醒、危险、成功、奖励和图表只在表达业务语义时使用各自语义色，不能为每个功能另造一套漂亮主色。
- **首页成员卡片例外**：首页宠物和人类卡片保留成员主题色与原有钱包卡片视觉，不能被全局黑白灰/lime 规则漂白。成员主题色只服务身份识别，不承担全局“点我”的职责。
...
- **功能 Icon**：全局功能 icon 使用 `monochromePrimary`。导航、按钮、设置行、快捷操作、列表和状态入口里的 icon 必须是 SF Symbol 或 template vector，统一使用小面积 `goLime`，不使用彩色仿真 icon、emoji、多色插画或拟物图标。
...
- **状态可读**：任何状态都必须靠文字、icon、颜色共同表达，不能只靠颜色；状态背景、badge、文字可以使用语义色，但功能 icon glyph 本身保持全局 `goLime` 单色。
- **状态优先于说明**：高频页面只展示当前状态和下一步动作，避免常驻“点击/长按/拖动/如何使用”式教程文案。
- **一个主节奏贯穿到底**：主操作 fill 统一使用黑/白 `goPrimary`，小面积确认、功能 icon、焦点和系统级选中态统一使用 `goLime`。业务色只表达业务含义，不承担全局“点我”的职责。
...
- `goPrimary`：全局 primary fill；浅色模式解析为黑，深色模式解析为白，用于大面积主 CTA 和需要稳定对比的 primary 控件。
- `goPrimary` 适用范围：主 CTA、确认按钮 fill、关键入口 fill 和 feeding stock/treat 的既定业务入口。
- `goLime`：保留为唯一全局小面积强调色；用于功能 icon、细描边、焦点环、选中线、小徽章和确认细节。宠物/人类主题色、业务模式色、图表系列色、计划等专有色不能直接复用。喂食模块中的余粮/库存/零食按业务决策使用 `goPrimary`。
- `goTeal`：完成、健康、成功。
- `goYellow`：进行中、注意、隐私。
- `goRed`：错误、危险、删除、异常。
- `goBlue`：历史兼容 token，当前解析为中性黑白灰，不再作为普通信息色或业务专有色使用。
- 成员主题色：只用于首页宠物/人类卡片、身份、图表系列、头像底色；从成员主题色库选择，必须避开 `goLime` 及 primary 明暗变体。旧数据若命中保留色，启动时自动归一到成员 fallback 色。
...
- 页面背景必须随深浅色变化。
- 浅色页面背景不能接近纯白；默认使用稍深的中性灰阶渐变，确保 `ohanaCardSurface`、白色卡片和输入块能从背景中分离。
- 官方背景和自定义照片背景都必须经过深浅色可读性遮罩。自定义照片只保存一张原图，渲染时按当前模式叠加黑白灰遮罩与轻模糊。
...
- Quick Action Circle：40x40 圆形 icon 按钮，功能 icon 统一使用小面积 `goLime` 单色；通过按钮背景、badge 或文字表达业务状态。
