...
> Status: confirmed on 2026-06-12. Product choices: Q1=A daily interval, Q2=A opportunistic lifecycle trigger without a new BGTask, Q3=A iCloud Drive ubiquity container file, Q4=A plaintext automatic JSON backup, Q5=A settings-visible failure plus gentle in-app reminder after repeated failures, Q6=A privacy reset removes Ohana-managed automatic backup files, Q7=A restore contract is backup-to-wipe-to-restore.
...
- Automatic backup content uses the same `OhanaBackup` projection as manual
  export.
...
exported or shared are not touched.

AB-011. Restore correctness for GAP-3 is defined as backup, wipe/reset to an
empty local store, restore, then verify that the backed-up data is complete. A
merge import into a non-empty store remains best-effort and is not the GAP-3
acceptance contract.
...
- Backup payload: reuse `DataBackupManager` / `DataBackupActor` so automatic
  backups cannot drift from manual export coverage.
...
- Reset: connect app reset/delete-all to automatic-backup cleanup if the reset
  service is already centralized; otherwise record the exact cleanup gap before
