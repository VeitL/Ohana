...
套餐的一句话定位固定为：Free“一个小型家庭可以真正完成基础照护”；Personal“一个人
长期照护时更省心、更深入、更长期”；Family“在 Personal 之上提供最多三位亲友的
Ohana App 内漏签守护”。本规则书只增加 Family 守护，不改变 Free / Personal 的本地价值。
...
截止加宽限前，只有本人明确轻点本人卡片、长按选分后松手，或解锁设备并点击本机通知“我今天平安”
动作，才通过 Presence 命令写事实。成功后清除连续漏签；打开或返回 App、前台激活、
后台刷新、同步、静默推送或服务端任务绝不能代替本人签到。首页不提供“一键全部”或
其他批量签到入口。

连续漏签状态机固定为：

1. 第 1 个有效漏签守护日创建事件，并最多提交一次首次 APNs；
2. 第 2 个有效漏签守护日最多提交一次跟进；
3. 同一事件此后保持安静，直到本人恢复、守护人确认或结构性停止。
...
信息，例如“连续 N 个守护日未收到打卡，请主动联系确认”。

## 补记、撤回与离线

- 参与区间内的过去漏签日只允许补记 1–10 分状态，不允许补写签到事实：不修复连续、
  不发奖励、不触发任何聚合完成态，也不改写已发生或已关闭的守护事件。
...
暂停、撤销、关系可达与签名权益，并用 DynamoDB 条件写防止并发重复提交。显式本人确认与
撤回完成可见 UI 交接后立即尝试发送 outbox；通知快捷确认同样立即发送。若用户在当天已明确
确认后才开启 Family 守护，启用事务必须补写该日既有明确确认的 outbox 信号，旧自动事实、
补记状态和恢复事实不得被提升为安全确认。
...
- 两个 Apple 测试账号、两台真机完成邀请、接受、第一个有效漏签守护日首次提醒、
  第二个有效漏签守护日唯一跟进、
