...
本规则书覆盖宪法 D2/D3/D7/D8/D12/D13/D14/D29/D30/D35/G2/G4/G5/G8 在 Economy 模块中的首发语义，并保留 GAP-4 总账恒等、GAP-5 触顶感知、GAP-7 补记结算的已验证规则。
...
- 上一条只适用于普通照护事实。佛系 Presence 历史只允许补记状态，不补写打卡事实，
  不进入照护补记奖励管线，也不产出椰子、里程碑或守护恢复信号。
- 佛系 Presence 的日常奖励只有两类：本人通过卡片或本机通知显式完成当日首次打卡
  `+1🥥 / +6 growthXP`，以及当天第一次在卡片明确选择状态 `+1🥥 / +2 growthXP`。
  椰子受操作日预算约束，成长 XP 不因预算触顶取消；App 生命周期不产出奖励，也没有
  “一键全部”或全部对象完成奖励。
...
- ECO-028（岛屿启动赠礼与 Oasis 注入）：D17 的一次性 50 椰子在第一位在世 Human 建立后进入待领取状态；Pet、Plant、首次打卡或照护均不参与资格。只有用户点击领取才由系统原子写入 `system:island`，不得绑定 Human/Pet。确定性交易键保证双击、跨模式、重启与崩溃恢复不重复发放；钱包 receipt 已存在但 defaults 或配对 CareLedger 投影丢失时必须恢复为已领取。交易提交即解锁 Oasis 与 Lv0 树，仪式不是解锁事实；赠礼不注入能量、不增加成长 XP，五次独立的 10 椰子注入才达到 Lv1。旧 v2 赠礼若已落入成员钱包，以一次性成对转账事实把当前仍可用的赠礼余额重分类到岛屿储备，并写幂等 marker；不得改写或删除旧流水。生命树注入可使用全部正式岛屿资产，固定按岛屿储备优先、当前 active Human 次优先、其余活跃成员稳定顺序出资；所有出资、注入 ledger 与能量事实必须同一事务提交，任何余额不足或保存失败都整体不生效。
- ECO-029（统一新手成长计划）：Standard 与 Zen 共用欢迎礼包 +50 和本人资料 75% +100 的资格、进度、receipt 与既有交易键；切换模式不能重置、重复展示已领取项或重复发奖。D28 六项家庭级一次性资料/成长任务仍合计 400 椰子，固定为 100 / 100 / 60 / 80 / 40 / 20，因此 Standard 总计为 50 + 400 = 450；Zen 只展示共享 Human 项，不要求 Pet/Plant，总计 150。Human 与首只 Pet 的资料任务各由四个等权类别组成，完成三类即达到 75% 领取门槛；姓名和所有默认值不构成资料完成。Human 的生日与性别/身份属于强制真实步骤：生日必须有日期，性别/身份必须有选择且允许选择“不愿透露”；这两个 checkpoint 不接受 skip resolution，只有两项均完成时才可领取 75% 奖励。其他非必填类别仍允许用暂无、不适用、不清楚、暂不填写或不愿透露代替填写；旧 Human 单一可选资料 checkpoint 只读映射到“其他资料”，不得补齐生日或性别。Plant 只显示完成度，不增加奖励。欢迎礼包进入 `system:island`；资料/成长奖励只在用户明确领取时写入当前有效 Human 钱包。它们不消耗每日照护预算、不增加成长 XP，也不得进入 `system:legacy`。领取前必须在同一 command 内重新验证真实资料、证件、保健、计划或照护事实；家庭 + 任务稳定交易键保证重复点击与崩溃恢复不重复发放。资格 checkpoint 只保存任务、选择结果和不透明目标 ID，不复制敏感资料值。已领取状态永久保留，后续删除资料不回收奖励。普通模式的首宠入口只是可关闭、零奖励建议，不进入任务数量、角标、奖励总额或 Oasis 解锁。旧“首次喂食 +15”和“主动主题色 +10”事实及历史流水保留兼容，但新行为不再产出这两笔隐藏奖励；既有成就奖励继续按其独立规则运行。
- ECO-030（佛系 Presence 奖励）：本人当日首次打卡 `+1🥥 / +6 growthXP` 只接受本人卡片
  轻点、长按选分松手或本机通知“我今天平安”等显式用户动作；前台激活、后台、同步与静默
  唤醒均不得领取。当天首次明确状态 `+1🥥 / +2 growthXP` 只接受当日卡片选分；它是家庭
  操作日级幂等奖励，后续状态编辑与历史状态补记不发奖。两项使生命树每天最多自然增加
  `8 growthXP`；其他对象打卡、批量/全部完成、里程碑与自动事件不增加成长。椰子预算触顶时
  允许实际发放为零，但同一原子事务仍写入对应成长事实。撤回当天 Presence 事实不回收已发
  椰子、成长事实或 receipt，同日再次显式打卡由 receipt 阻止重复发放。
...
- 佛系 Presence 历史状态补记明确排除在上一条之外；它只保存状态，不是签到或照护补记。
