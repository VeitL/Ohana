...
本规则书覆盖宪法 D2/D3/D7/D8/D12/D13/D14/D29/D30/G2/G4/G5/G8 在 Economy 模块中的首发语义，并保留 GAP-4 总账恒等、GAP-5 触顶感知、GAP-7 补记结算的已验证规则。
...
- 佛系 Presence 的日常奖励只有两类：本人通过卡片或本机通知显式完成当日首次打卡
  `+1`，以及当天第一次在卡片明确选择状态 `+1`。App 生命周期不产出奖励；没有
  “一键全部”或全部对象完成 `+2` 奖励。
...
- ECO-030（佛系 Presence 奖励）：本人当日首次打卡 `+1` 只接受本人卡片轻点、长按选分
  松手或本机通知“我今天平安”等显式用户动作；前台激活、后台、同步与静默唤醒均不得领取。
  当天首次明确状态 `+1` 只接受当日卡片选分；历史状态补记不发奖。不存在批量打卡或
  全部对象完成奖励。撤回当天事实不回收已发椰子或删除 receipt，同日再次显式打卡由
  receipt 阻止重复发放。
