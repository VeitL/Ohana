...
- OAS-006：生命树等级由账本中的 `growthXP`、显式 `treeInjection` / `treeInjectionLarge` 事实的 `injectedXP` 与兼容 baseline 推导；首次接入以既有历史成长为 baseline，之后新增照护成长可计入。只有显式注入事实可贡献 `injectedXP`，同一事实不得重复增加树 XP。
