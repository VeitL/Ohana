...
- 当前 Family 守护调度为禁用实现，不注册 APNs。未来顺序固定为本机提醒 → 宽限期
  → 服务端通知已安装且接受邀请的 Ohana 成员；不得自动短信或邮件外部联系人。
...
- NTF-009：只有 `PRESENCE_OKAY` 等明确通知动作可发出本人打卡意图；重复动作按
  主体/自然日幂等。
- NTF-010：短信只能打开系统编辑器并预填，由用户确认。系统未返回 sent 时不得称
  已送达；不可发送时提供复制号码/文案。固定语义为“未收到今天的打卡，请主动确认”，
  不得宣称急救、死亡检测或保证送达。
...
- V93 只为本机手填安全联系人增加 `SafetyContact`；提醒时间、星期、宽限期、第二次提醒
  和短信模板仍是设备本地设置，不进入备份或同步。
- 本轮不启用 CloudKit、远程推送、后台推送或联机协作通知。
- 设置提供独立佛系提醒与联系人页面；只有用户点击保存并启用时才请求通知权限。
  真实系统通知授权、到达、点击动作和短信结果仍必须在真机验收。
