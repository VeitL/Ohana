...
`actionRequired`，点击后直接进入大尺寸、item-driven 的逐题卡片向导，而不是先展示
说明页。向导一次只聚焦一个 checkpoint，显示题目进度，并按当前问题把主操作路由到
现有 Human / Pet 资料、证件、健康、喂养计划或手动照护页面；例如证件问题进入证件页，
紧急联系人问题进入 Pet 基本资料，日常照护问题进入喂养设置。完成一题后向导移动到
下一项未完成问题。Human 与 Pet 档案都固定四类，每类 25%，达到 75%（任意三类）即
进入可领取状态；任务进度显示真实百分比与 75% 门槛，不再把四类压缩成虚假的二项进度。
对应编辑或记录页面作为向导之上的
临时 editor 呈现；成员基础资料直接进入编辑态，关闭 editor 后必须保留原向导上下文，
所有入口在保存出真实、符合当前 checkpoint 的事实后都自动关闭 editor，重新读取完成
事实并继续下一题；取消、普通关闭或无关保存不得误推进，也不得把用户丢回普通详情或
结束整段引导。

资格满足后同一稳定 ID 切换为 `rewardReady`。用户必须再次点击该行，在同一
item-driven sheet 中确认领取；保存资料、记录 checkpoint 或照护事实本身不弹奖励层，
也不自动发奖。该组只进入清单，不进入日历，不受默认成员筛选隐藏，也不显示发布者、
执行者或审核人控件。

每张可选问题卡都允许用户以“保持现状 / 暂无 / 不适用 / 不清楚 / 不愿透露”等该
checkpoint 白名单内的明确选择完成确认；这些答案必须与填写资料入口在同一卡片内直接
可见，不得藏在次级菜单或以奖励文案诱导披露。
Human 四类为形象、生日阶段、身份/身体资料、性格/地区/故事；Pet 四类为生日/到家日、
身体资料、形象/性格、日常照护。旧 `humanOptionalDetails` 只读映射到后三个 Human 类别，
不回写、不迁移。所有基础资料页统一显示四类完成度；Plant 只显示物种/形象、获得信息、
位置/明确环境、盆土/尺寸/备注四类完成度，不进入新手奖励。
