...
struct PlantDashboardLongLanguageLayoutTests {
    ...
    @Test func overviewQuickActionsUseAdaptiveGridAfterStatusRibbonRetirement() throws {
        let source = try source(
            "Ohana/Features/Plants/Views/PlantDashboardView+Sections.swift",
            rootURL: repositoryRootURL()
        )

        #expect(!source.contains("dashboardStatusLine"))
        #expect(!source.contains("dashboardStatusRibbon"))
        #expect(!source.contains("dashboardStatusColumns"))
        #expect(!source.contains("dashboardStatusChip"))
