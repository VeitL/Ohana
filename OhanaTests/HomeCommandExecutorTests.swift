...
struct HomeCommandExecutorTests {
    ...
    @Test func plantQuickCareHomeSurfaceTracksPendingCompletedAndRewardFeedback() throws {
        ...
        #expect(plantCommandSource.contains("context.safeSaveResult(publishFailureEvent: true)"))
        #expect(plantCommandSource.contains("context.rollback()"))
    ...
    @Test func petCareFactsStopDerivedEffectsWhenPersistenceFails() throws {
        ...
        let sharedCareSource = try source("Ohana/Domain/Services/CareEventService+Shared.swift", rootURL: rootURL)
        ...
        let quickFeedExecutorSource = try source("Ohana/Features/Feeding/QuickFeedCommandExecutor.swift", rootURL: rootURL)
        let quickFeedActionsSource = try source("Ohana/Features/Feeding/Views/QuickFeedDetailContent+FeedingCommandActions.swift", rootURL: rootURL)
        let expandedQuickActionSource = try source("Ohana/Features/Home/ExpandedQuickActionExecutor.swift", rootURL: rootURL)
        ...
        #expect(sharedRecorderSource.contains("context.safeSaveResult(publishFailureEvent: true)"))
        #expect(sharedRecorderSource.contains("guard saveResult.didSave else"))
        #expect(sharedCareSource.contains("didPersist: recorded.result.didPersist"))
        #expect(sharedCareSource.contains("persistenceErrorDescription: recorded.result.persistenceErrorDescription"))
        #expect(sharedCareSource.contains("targetPetIDs: didPersist && disposition.didWriteFact ? [target.id] : []"))
        ...
        #expect(feedFactSource.contains("context.safeSaveResult(publishFailureEvent: true)"))
        #expect(feedCommandSource.contains("let didPersist: Bool"))
        #expect(feedCommandSource.contains("let persistenceErrorDescription: String?"))
        #expect(feedCommandSource.contains("guard recorded.didPersist else"))
        #expect(feedCommandSource.contains("guard completed.didPersist else"))
        #expect(feedCommandSource.contains("didPersist: recorded.result.didPersist"))
        #expect(feedCommandSource.contains("persistenceErrorDescription: recorded.result.persistenceErrorDescription"))
        #expect(quickFeedExecutorSource.contains("wroteBusinessFact: result.didPersist && result.didRecord && result.allowsDerivedEffects"))
        #expect(quickFeedActionsSource.contains("guard result.didPersist else"))
        #expect(quickFeedActionsSource.contains("showFeedPersistenceFailure()"))
        #expect(expandedQuickActionSource.contains("guard result.didPersist, result.didRecord, result.allowsDerivedEffects else"))
    ...
    @Test func plantCarePlanScheduleCommitsExternalEffectsOnlyAfterPersistence() throws {
        ...
        #expect(scheduleSource.contains("typealias PlantCarePlanPersistenceSave"))
        #expect(scheduleSource.contains("persistenceSave: PlantCarePlanPersistenceSave = { context in"))
        #expect(scheduleSource.contains("context.safeSaveResult(publishFailureEvent: true)"))
        #expect(scheduleSource.components(separatedBy: "context.safeSaveResult(publishFailureEvent: true)").count - 1 == 1)
        #expect(scheduleSource.components(separatedBy: "let saveResult = persistenceSave(context)").count - 1 == 2)
        ...
        #expect(lifecycleSource.components(separatedBy: "context.safeSaveResult(publishFailureEvent: true)").count - 1 == 2)
    ...
    @Test func familyTaskCommandsGateRevisionAndExternalEffectsOnPersistence() throws {
        ...
        let reminderSyncSource = try source("Ohana/Features/FamilyTasks/FamilyTaskService+ReminderSync.swift", rootURL: rootURL)
        ...
        #expect(serviceSource.contains("static func persistMutation("))
        ...
        #expect(reminderSyncSource.contains("return persistMutation(context: context)"))
        #expect(!reminderSyncSource.contains("context.safeSave()"))
    ...
    @Test func memberCreationOnlyWritesExplicitSchedulesAndDefersRevisionsUntilPersistence() throws {
        let rootURL = repositoryRootURL()
        ...
        #expect(!creationSource.contains("CarePlanCalendarSync.ensureDefaultPlans("))
        #expect(!creationSource.contains("defaultPlanSideEffects"))
    ...
    @Test func petMedicationCommandsGateSideEffectsOnPersistence() throws {
        let rootURL = repositoryRootURL()
        let commandSource = try source("Ohana/Features/Medication/MedicationCommands.swift", rootURL: rootURL)
        let doseSource = try source("Ohana/Domain/Services/PetMedicationDoseLogging.swift", rootURL: rootURL)
        let homeExecutorSource = try source("Ohana/Features/Home/HomeCommandExecutor.swift", rootURL: rootURL)
        let quickExecutorSource = try source("Ohana/Features/Home/ExpandedQuickActionExecutor.swift", rootURL: rootURL)
        let humanExecutorSource = try source("Ohana/Features/HumanNotes/HumanNoteCommands.swift", rootURL: rootURL)
        let petRevisionSource = try source("Ohana/Features/RevisionPublishing/DomainRevisionPublishing+FeaturePublishing.swift", rootURL: rootURL)
        let humanRevisionSource = try source("Ohana/Features/RevisionPublishing/DomainRevisionPublishing+HumanPublishing.swift", rootURL: rootURL)
        ...
        #expect(commandSource.contains("context.safeSaveResult(publishFailureEvent: true)"))
        #expect(commandSource.contains("context.rollback()"))
        #expect(commandSource.contains("PendingMedicationStorageWrite"))
        #expect(commandSource.contains("DeferredMedicationNotificationCanceller"))
        #expect(commandSource.contains("deferredNotifications.commit(to: OhanaNotifications.current)"))
        #expect(commandSource.contains("guard result.didPersist else { return result }"))
        #expect(doseSource.contains("let didPersist: Bool"))
        #expect(doseSource.contains("let persistenceErrorDescription: String?"))
        #expect(doseSource.contains("modelContext.safeSaveResult(publishFailureEvent: true)"))
        #expect(doseSource.contains("modelContext.rollback()"))
        #expect(!doseSource.contains("try modelContext.save()"))
        #expect(doseSource.contains("pendingRemainingAmountWrite.commit()"))
        #expect(homeExecutorSource.contains("guard result.didPersist, result.didRecord, result.allowsDerivedEffects else"))
        #expect(quickExecutorSource.contains("guard result.didPersist, result.didRecord, result.allowsDerivedEffects else"))
        #expect(humanExecutorSource.contains("guard result.didPersist else { return result }"))
        #expect(petRevisionSource.contains("wroteBusinessFact: result.didPersist"))
        #expect(petRevisionSource.contains("wroteBusinessFact: result.didPersist && result.didChange"))
        #expect(humanRevisionSource.contains("wroteBusinessFact: result.didPersist"))
    }
    ...
    @MainActor
    @Test func humanExpenseServiceRejectsHumanSubjectWithoutWritingFactsOrLedger() throws {
        let container = try makeInMemoryContainer()
        let context = container.mainContext
        let human = Human(name: "Guan")
        let recorder = Human(name: "Alex")
    ...
    @MainActor
    @Test func petExpenseServiceStoresMultiplePayersOnceAndRewardsPrimaryOnce() throws {
        let container = try makeLatestInMemoryContainer()
        let context = container.mainContext
        let pet = Pet(name: "Momo", species: "猫")
        ...
        let cleanup = isolateSharedCareEconomy(activeHumanID: primaryPayer.id.uuidString)
        defer { cleanup() }
        let contributions = [
        ...
        let result = try ExpenseCommandService.recordPetExpense(
            ...
            context: context,
            executorId: coPayer.id.uuidString,
            recordedByHumanId: recorder.id.uuidString,
        ...
        let expenseLedgers = try context.fetch(FetchDescriptor<CareLedgerEvent>()).filter {
            $0.eventKind == CareLedgerEventKind.expense.rawValue
        }
        let rewardEntries = try context.fetch(FetchDescriptor<CoconutLedgerEntry>()).filter {
            $0.entryKind == .reward && $0.delta > 0
        }

        #expect(try context.fetch(FetchDescriptor<PetExpenseLog>()).count == 1)
        #expect(expense.id == result.logID)
        #expect(expense.executorId == primaryPayer.id.uuidString)
        #expect(expense.recordedByHumanId == recorder.id.uuidString)
        #expect(expense.payerContributions == contributions)
        #expect(expense.amountPaid(by: primaryPayer.id.uuidString) == 60)
        #expect(expense.amountPaid(by: coPayer.id.uuidString) == 40)
        #expect(expenseLedgers.count == 1)
        #expect(expenseLedgers.first?.actorId == primaryPayer.id.uuidString)
        #expect(expenseLedgers.first?.amountValue == 100)
        #expect(rewardEntries.count == 1)
        #expect(rewardEntries.first?.ownerId == primaryPayer.id.uuidString)
        #expect(rewardEntries.first?.delta == result.coconutDelta)
        #expect(!rewardEntries.contains { $0.ownerId == coPayer.id.uuidString })
    }

    @MainActor
    @Test func petExpenseServiceRejectsInactivePayerBeforeWritingAnyFacts() throws {
        let container = try makeLatestInMemoryContainer()
        let context = container.mainContext
        let pet = Pet(name: "Momo", species: "猫")
        let primaryPayer = Human(name: "Guan")
        let inactivePayer = Human(name: "Mia")
        inactivePayer.passedAwayDate = Date(timeIntervalSince1970: 1_700_000_000)
        context.insert(pet)
        context.insert(primaryPayer)
        context.insert(inactivePayer)
        try context.save()
        ...
        let contributions = [
            ExpensePayerContribution(humanID: primaryPayer.id, minorUnits: 6000),
            ExpensePayerContribution(humanID: inactivePayer.id, minorUnits: 4000)
        ]

        #expect(throws: ExpensePayerContributionError.inactivePayer) {
            try ExpenseCommandService.recordPetExpense(
                pet: pet,
                amount: 100,
    ...
    @Test func petMedicationPlanServiceCreatesUpdatesAndDeletesPlan() throws {
        ...
        #expect(created.didPersist)
        #expect(created.persistenceErrorDescription == nil)
        ...
        #expect(activated.didPersist)
        #expect(activated.persistenceErrorDescription == nil)
        #expect(medication.isActive == true)
        events = try context.fetch(FetchDescriptor<Event>())
        #expect(events.count == 1)
    ...
    @Test func petMedicationCommandExecutorPublishesCreateActivationAndDeleteRevisions() throws {
        ...
        #expect(activation.didChange == true)
        #expect(activation.didPersist)
        #expect(activation.persistenceErrorDescription == nil)
        #expect(medication.isActive == false)
    ...
    @MainActor
    @Test func dashboardRecordCommandExecutorPublishesWeightRevisionsAndRejectsHumanExpense() throws {
        let revisionCenter = ReadModelRevisionCenter()
        let container = try makeInMemoryContainer()
        let context = container.mainContext
        let pet = Pet(name: "Momo", species: "猫")
        ...
        #expect(throws: ExpenseSubjectPolicyError.humanSubjectNotSupported) {
            _ = try executor.recordHumanExpense(
                human: human,
                amount: 12.5,
        ...
        #expect(revisionCenter.lastMutation == weightDeleteMutation)
        #expect(revisionCenter.homeRevision.value == beforeRevision + 2)
        #expect(try context.fetch(FetchDescriptor<PetExpenseLog>()).isEmpty)
    }
