...
struct QuickFeedDataControllerTests {
    ...
    @Test func feedTodayStateDoesNotFallbackToPetCareLogRelationship() throws {
        ...
        #expect(planWriterSource.contains("let readableEvents = QuickFeedModelReadability.readableEvents(allEvents)"))
        #expect(planWriterSource.contains("let stockLedgerEntries = feedingLedgerEntries(pet: pet, allEvents: readableEvents, context: context)"))
